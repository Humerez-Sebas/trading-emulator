...
- Memory: all 8 same-symbol panels reference **1** `Candle[]` array (measured `===`), i.e. O(1) in panel count — **[confirm: PASS — `chart-model-mapper.eight-panel-profile.spec.ts`, test 1, measured 2026-07-04]**.
- Render fan-out, 8 visible: **[record: measured 8 total `panelChartView$` recomputes for one clock tick across 8 visible panels — 1 additional emission per panel (count went 1→2 per mapper), sum = 8, exactly `MAX_PANELS_PER_TAB`, NOT 8×8 = 64 — linear, confirmed by `chart-model-mapper.eight-panel-profile.spec.ts`, test 2, PASS on first run]**.
- Render fan-out, 3 of 8 hidden: **[record (corrected by final audit): the 3 hidden panels contributed 0 emissions — gated BEFORE subscribe, `gated()`'s `enabled` filter suppresses even their initial emission — so the test's plan-verbatim `− MAX_PANELS_PER_TAB` baseline computes `rendersThisTick = (5 visible × 2 emissions) − 8 = 2` under the `toBeLessThanOrEqual(5)` bound. The bound is genuinely discriminating: an UNGATED variant would compute (8 × 2) − 8 = 8 and FAIL `8 ≤ 5`, so the load-bearing claim "hidden panels contribute zero render work per tick" is proven; the per-tick render-stream count with 3 of 8 gated is bounded by the visible-panel count (≤ 5), never more — confirmed by `chart-model-mapper.eight-panel-profile.spec.ts`, test 3 (counts `chartView$`, the gated render stream, per the line-538 note — `selectChartView` overridden in `beforeEach` with the real projection shape from `chart-model-mapper.service.ts:166-173`), PASS on first run]**.
- Conclusion: **[record: fan-out is linear in *visible* panel count — 8 visible panels produce exactly 8 recomputes per tick (not quadratic), and gating 3 of 8 caps the render-stream count at 5 (≤ visible-panel count), never more]**. Fan-out is linear in *visible* panel count; update-gating + first-show lazy creation bound both compute and engine-instance count; the measured per-tick work stays well within the 16 ms/frame budget projected to 8 panels. **No Web Worker (D7) and no second candle cache (R4) are introduced** — the profiling documents no gap that would justify them.
