...
- Render fan-out, 3 of 8 hidden: **[record: measured 5 `chartView$` render-stream emissions for one clock tick with 3 of 8 panels gated via `setUpdatesEnabled(false)` — the 5 visible panels each contributed exactly 1 emission, the 3 hidden panels contributed 0 (gated() suppresses via the `enabled` filter), rendersThisTick = 5 ≤ visiblePanels = 5 — confirmed by `chart-model-mapper.eight-panel-profile.spec.ts`, test 3 (counts `chartView$`, the gated render stream, per the line-538 note — `selectChartView` was overridden in `beforeEach` with the real projection shape from `chart-model-mapper.service.ts:166-173`; wiring was not fiddly so `chartStyle$` fallback was not needed), PASS on first run]**.
