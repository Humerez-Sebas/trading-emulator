...
**Estado:** aprobado por el propietario el 2026-08-05.
...
## Decision D.20.5: unidad operativa derivada por instrumento
...
| Metal XAU/XAG con `pointSize` MT5 conocido | `pts` | `pointSize` |
| Indice y demas no-FX | `pts` | `1` |
...
Un simbolo heuristico no tiene datos MT5 verificables. Si un metal heuristico
carece de `pointSize`, conserva el fallback existente de `1` y muestra su
marca de procedencia heuristica; esta decision no inventa un punto tecnico ni
agrega copy o estados de error fuera del alcance aprobado.
...
| `emulador/src/app/lotaje/sizing-view-model.ts` | Aplicar D.20.5 en ambos sentidos. |
...
| `.superpowers/rfc-020/dev-log.md` | Registrar D.20.5, la excepcion de rama y evidencia. |

## Especificaciones preexistentes y excepcion de rama

La autoridad del propietario permite cambiar exclusivamente los pines
preexistentes que afirmen que XAUUSD en modo Distancia usa unidades de precio
crudas. Cada eliminacion debe tener un sucesor nombrado que pruebe la nueva
semantica completa. El caso de aceptacion US30 permanece sin cambios.
...
- El registro generado contiene el `pointSize` leido de MT5 para todos los
  simbolos cosechados.
- US30 conserva su unidad de punto indice y XAUUSD usa puntos MT5.
