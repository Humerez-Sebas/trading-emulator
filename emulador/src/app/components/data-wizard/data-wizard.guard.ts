...
/**
 * Routes a first-time R2 user to the Markets R2 data hub on first launch.
 *
 ...
 * user is redirected to `/mercados` (the R2 hub, where data is downloaded); once
 * any dataset exists, the emulator loads normally. The `/mercados` route does
 * NOT carry this guard, so there is no redirect loop.
 */
