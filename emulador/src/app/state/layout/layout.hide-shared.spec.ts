...
describe('layout reducer: setPanelHideSharedDrawings', () => {
  it('toggling sets the flag to true', () => {
    ...
    expect(next.panels['panel-1'].hideSharedDrawings).toBe(true);
  });
  ...
  it('toggling back off CLEARS the field rather than writing `false` explicitly', () => {
    ...
    expect(state.panels['panel-1'].hideSharedDrawings).toBe(true);
    ...
    expect('hideSharedDrawings' in next.panels['panel-1']).toBe(false);
  });
  ...
  it('is a no-op (identity return) for an unknown panelId', () => {
    ...
    const next = reducer(
      state,
      LayoutActions.setPanelHideSharedDrawings({ panelId: 'nope', hidden: true }),
    );
  ...
  it('the flag is absent (not `false`) on descriptors that never toggled', () => {
    const state = createInitialLayoutState();
    expect('hideSharedDrawings' in state.panels['panel-1']).toBe(false);
  });
  ...
  it('leaves every other descriptor field untouched', () => {
    ...
    expect(next.panels['panel-1']).toEqual({ ...before, hideSharedDrawings: true });
  });
});
