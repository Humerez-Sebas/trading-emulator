...
describe('linkGroupsFeature reducer', () => {
  ...
  it('createGroup adds a group; is a no-op on duplicate id', () => {
    ...
    const again = reducer(
      state,
      LinkGroupsActions.createGroup({ group: { ...group('g1'), color: '#000' } }),
    );
  ...
  it('setSyncCrosshair / setSyncTimeRange toggle independently; no-op on unknown group', () => {
    ...
    expect(
      reducer(state, LinkGroupsActions.setSyncCrosshair({ groupId: 'nope', enabled: true })),
    ).toBe(state);
  });
  ...
  it('accepts and stores the reserved syncPriceScale field without interpreting it', () => {
    ...
    const state = reducer(
      createInitialLinkGroupsState(),
      LinkGroupsActions.createGroup({ group: withReserved }),
    );
  ...
  describe('restoreGroups (RFC-011 Task 2)', () => {
    it('replaces the entire groups map keyed by id', () => {
      ...
      const state = reducer(
        createInitialLinkGroupsState(),
        LinkGroupsActions.restoreGroups({ groups }),
      );
    ...
    it('hydrates groups from the restored Workspace.linkGroups when present', () => {
      ...
      const state = reducer(
        ...
        WorkspacesActions.workspaceRestored({
          workspace: { ...emptyWorkspace('EURUSD'), linkGroups: groups },
        }),
      );
